...
export default function (files, options) {
  ...
  const pkgName = options.name + '-' + options.version + '-' + options.release + '.' + options.buildArch;
  const specFile = path.join(options.tempDir, 'SPECS', pkgName + '.spec');
  ...
  _.forEach(files, function (file) {
    const directive = (typeof file.directive === 'undefined') ? '' : '%' + file.directive + ' ';
